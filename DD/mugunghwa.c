...
void turn_of_camera() {
    ...
    /// 움직인 플레이어 저장(0번째 인덱스부터 순차 저장될 예정.
    ...
    /// 3초동안 while문이 실행됩니다. 이 동안에 움직인 플레이어가 있으면 해당 플레이어를 파악해야합니다. 
    while ((time(NULL) < endTime)) {
        ...
        /// before_players_moving_info_buf와 back_buf에서 이동된 사용자를 찾습니다. 
        for (int i = 0; i < ROW_MAX; i++) {
            for (int j = 0; j < COL_MAX; j++) {
                ...
                if (player != '-1' && (before_players_moving_info_buf[i][j] != back_buf[i][j])) {
                    /// i,j를 기준으로 상, 하, 좌, 우를 파악해서 움직이기 이전 정보에 player와 일치하는 플레이어가 있다는가? 
                    /// 있다면 해당 플레이어는 움직인 것 검거 완료.
